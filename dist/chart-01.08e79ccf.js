...
function ready(_ref) {
  ...
  var areas2 = topojson.feature(json, json.objects.Beijing2);
  console.log('areas are', areas2.features);
  projection.fitSize([width, height], areas2);
  svg.append('g').selectAll('.area2').data(areas2.features).enter().append('path').attr('class', 'area2').attr('d', path).attr('stroke', 'grey').attr('fill', 'none');
  svg.append('g').selectAll('circle').data(datapoints).enter().append('circle').attr('r', function (d) {
    return radiusScale(d.y15);
